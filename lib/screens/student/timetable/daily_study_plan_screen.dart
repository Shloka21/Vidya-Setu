...
class _DailyStudyPlanScreenState extends State<DailyStudyPlanScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Daily Study Plan', style: Theme.of(context).textTheme.headlineSmall),
      ),
      body: Column(
        children: [
          _buildDateHeader(),
  ...
  Widget _buildDateHeader() {
    final totalHours = _sessions.fold(0, (sum, session) => sum + session.durationMinutes) / 60;
  ...
  Widget _buildProgressBar() {
    ...
    int totalMinutes = _sessions.fold(0, (sum, s) => sum + s.durationMinutes);
